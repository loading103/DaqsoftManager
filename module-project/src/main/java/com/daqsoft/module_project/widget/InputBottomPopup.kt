...
import com.daqsoft.module_project.adapter.AnnexAdapter
import com.daqsoft.module_project.adapter.LabelAdapter
 ...
 * @describe 输入框 弹窗
 */
class InputBottomPopup  : BottomPopupView {
    ...
    val annexAdapter : AnnexAdapter by lazy { AnnexAdapter() }
    ...
    val itemBinding : ItemBinding<String> = ItemBinding.of{ itemBinding, position, item ->
        when (item) {
            ConstantGlobal.FILE -> itemBinding.set(
                ...
                R.layout.recyclerview_project_dynamic_annex_file
            )
            ConstantGlobal.IMAGE -> itemBinding.set(
                ItemBinding.VAR_NONE,
                R.layout.recyclerview_project_dynamic_annex_image
            )
            ConstantGlobal.LEDGER -> itemBinding.set(
                ItemBinding.VAR_NONE,
                R.layout.recyclerview_project_dynamic_annex_ledger
            )
    ...
    override fun getImplLayoutId(): Int {
        return R.layout.layout_popup_input
    }
...
//                    labelAdapter.checkLabelContains(text.toString())
        ...
        send.setOnClickListener {
            ...
            onClickListener?.sendOnClick(
                ...
                aTailData.joinToString(separator = ",") { it.id.toString() },
                replyId,
                noteId
            )
        }
        ...
        labelRecyclerView.apply {
            ...
            adapter = labelAdapter.apply {
                itemBinding = ItemBinding.of(BR.content, R.layout.recyclerview_input_popup_item)
                setItems(labelData)
                setItemOnClickListener(object : LabelAdapter.ItemOnClickListener {
...
//                        }
        ...
        annexRecyclerView?.run {
            ...
            adapter = annexAdapter.apply {
                itemBinding = this@InputBottomPopup.itemBinding
                setItems(this@InputBottomPopup.observableList)
                setItemRemoveListener(object : AnnexAdapter.OnClickListener {
...
//            }
    ...
    fun setLedgerData(data: MutableList<AccountBackBean>){
        ...
        annexAdapter.setLedgerData(ledgerData)
        annexAdapter.setItems(observableList)
        annexAdapter.notifyDataSetChanged()
        annexRecyclerView?.smoothScrollToPosition(observableList.size - 1)
    }
