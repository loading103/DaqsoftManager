...
import com.daqsoft.module_workbench.widget.enums.SelectType;
...
public class LabelsView extends ViewGroup implements View.OnClickListener, View.OnLongClickListener {
    ...
    public boolean isNeedToastMaxTip = false;
    ...
    private OnLabelShowMoreListener mOnLabelShowMoreListener;
    ...
    private void getAttrs(Context context, AttributeSet attrs) {
        if (attrs != null) {
            ...
            if (mTypedArray.hasValue(R.styleable.labels_view_labelBackground)) {
                ...
                mLabelBg = getResources().getDrawable(R.drawable.home_b_f5_stroke_null_round_5);
            }
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (lineWidth + view.getMeasuredWidth() > maxWidth) {
                ...
                if (mMaxLines > 0 && lineCount > mMaxLines) {
                    if (mOnLabelShowMoreListener != null) {
                        mOnLabelShowMoreListener.onLabeShowLoadMore();
                    }
                    isNeedShowMore = true;
                    break;
                }
...
//            }
     ...
     * @param labels
     */
    public void setLabels(List<String> labels, int postion) {
        setLabels(labels, new LabelTextProvider<String>() {
            @Override
            public CharSequence getLabelText(TextView label, int position, String data) {
                return data.trim();
     ...
     * @param <T>
     */
    public <T> void setLabels(List<T> labels, LabelTextProvider<T> provider, int postion) {
        //清空原有的标签
        innerClearAllSelect();
        removeAllViews();
        mLabels.clear();
                ...
                if (mSelectType != SelectType.NONE) {
                    if (label.isSelected()) {
                        ...
                        if (mSelectType == SelectType.SINGLE || mSelectType == SelectType.SINGLE_IRREVOCABLY) {
                            ...
                            if (mMaxSelect <= 0 || mMaxSelect > mSelectLabels.size()) {
                                if (!selectChangeIntercept(label)) {
                                    setLabelSelect(label, true);
                                }
                            } else {
                                if (isNeedToastMaxTip) {
                                    Toast.makeText(mContext, "最多选择" + mMaxSelect + "个标签", Toast.LENGTH_SHORT).show();
                                }
                            }
                        }
                    }
    ...
    public void updateFisrtLable(String content) {
        int count = getChildCount();
        if (count > 0) {
            try {
    ...
    public void setMaxSelectUnChange(int maxSelect) {
        if (mMaxSelect != maxSelect) {
            mMaxSelect = maxSelect;
        }
    }
    ...
    public void setmOnLabelShowMoreListener(OnLabelShowMoreListener l) {
        mOnLabelShowMoreListener = l;
    }
    ...
    public interface OnLabelShowMoreListener {
        /**
         * 展示更多
         */
        void onLabeShowLoadMore();
    }
