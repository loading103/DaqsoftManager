 ...
 * @author zp
 * @describe 部门文件搜索 viewModel
 */
class DeptDocSearchViewModel : ToolbarViewModel<WorkBenchRepository>{
    ...
    var searchTextChanged = BindingCommand<String>(BindingConsumer {
        ...
        when(type){
            "部门文件" ->{
                getDeptOrPublicFileInfo(null,null,0,it)
            }
            "公共文件" ->{
                getDeptOrPublicFileInfo(null,null,1,it)
            }
            "共享文件" ->{
                getShareFileInfo(null, deptId?.toInt(),it)
            }
        }
    })
                ...
                .subscribeWith(object : AppDisposableObserver<AppResponse<DeptFile>>() {
                    override fun onSuccess(t: AppResponse<DeptFile>) {
                        t.data?.let {
                            ...
                            it.folders.forEach {
                                val itemViewModel = DeptDocFolderItemViewModel(this@DeptDocSearchViewModel,it)
                                itemViewModel.multiItemType(ConstantGlobal.FOLDER)
                                observableList.add(itemViewModel)
                            }

                            it.files.forEach {
                                val itemViewModel = DeptDocFileItemViewModel(this@DeptDocSearchViewModel,it)
            ...
            model
                ...
                .subscribeWith(object : AppDisposableObserver<AppResponse<Any>>() {
                    override fun onSuccess(t: AppResponse<Any>) {
                        when(type){
                            "部门文件" ->{
                                getDeptOrPublicFileInfo(null,null,0,searchObservable.get())
                            }
                            "公共文件" ->{
                                getDeptOrPublicFileInfo(null,null,1,searchObservable.get())
                            }
                            "共享文件" ->{
                                getShareFileInfo(null, deptId?.toInt(),searchObservable.get())
                            }
                        }
                    }
                })
