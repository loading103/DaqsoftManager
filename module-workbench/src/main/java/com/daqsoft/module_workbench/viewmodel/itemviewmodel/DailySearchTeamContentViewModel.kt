 ...
 */
class DailySearchTeamContentViewModel(
    private val dailyListViewModel: DailySearchTeamViewModel,
    it: DialyRecord
) : MultiItemViewModel<DailySearchTeamViewModel>(dailyListViewModel){
    ...
    val todayAdd = ObservableField<Boolean>(false)
    ...
    val onSubmitClick = BindingCommand<Unit>(BindingAction {
        dailyListViewModel.setItemViewModel(this@DailySearchTeamContentViewModel)
        goAddView()
    })
    ...
    private fun handleData(it: DialyRecord) {
        ...
        if(it.reportDate == currentData){
            ...
            val haveSubmit =
                it.dailyCheck.isNullOrEmpty() && it.todayInfo.isNullOrEmpty() && it.tomorrowPlan.isNullOrEmpty() && it.needHelp.isNullOrEmpty()
            if (!haveSubmit) {
                todayEdit.set(View.GONE)
                noSubmitVisible.set(false)
                todayAdd.set(false)
