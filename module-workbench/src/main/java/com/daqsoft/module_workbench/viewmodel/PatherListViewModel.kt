...
import com.daqsoft.module_workbench.viewmodel.itemviewmodel.PatherItemViewModel
import com.daqsoft.mvvmfoundation.base.BaseApplication
import com.daqsoft.mvvmfoundation.utils.RxUtils
import com.google.gson.Gson
import com.luck.picture.lib.tools.ToastUtils

class PatherListViewModel : ToolbarViewModel<WorkBenchRepository>,
    ...
    var itemBinding: ItemBinding<ItemViewModel<*>> = ItemBinding.of(
        BR.viewModel,
        R.layout.recycleview_pather_item
    )
    ...
    override fun rightIcon2OnClick() {
        ...
        ARouter.getInstance().build(ARouterPath.Workbench.PAGER_PATERNER_SEARCH)
    ...
    fun getTypeList(){
        addSubscribe(
            model.getTypeList(typeClassify = "1")
    ...
    fun getLevelList(){
        addSubscribe(
            model.getGradeList(gradeClassify = "1")
                .compose(RxUtils.exceptionTransformer())
                .compose(RxUtils.schedulersTransformer())
        ...
        dataSource = object : PageKeyedDataSource<Int, ItemViewModel<*>>(){
            override fun loadInitial(
                ...
                addSubscribe(
                    model
                        .getParnterList(
                            partnerType =chooseType?.value?.id,
                            partnerGrade = chooseLevel?.value?.id
                        )
                        .compose (RxUtils.schedulersTransformer())
                        .compose (RxUtils.exceptionTransformer())
                            ...
                            override fun onSuccess(t: AppResponse<PartnerBean>) {
                                ...
                                t.data?.let {
                                    ...
                                    it.records?.forEach {it1->
                                        observableList.add(PatherItemViewModel(this@PatherListViewModel, it1,typeDatas.value))
                                    }
                        ...
                        .subscribeWith(object : AppDisposableObserver<AppResponse<PartnerBean>>() {
                            override fun onSuccess(t: AppResponse<PartnerBean>) {
                                t.data?.let {
                                    val observableList: ObservableList<ItemViewModel<*>> = ObservableArrayList()
                                    it.records?.forEach {it1->
                                        observableList.add(PatherItemViewModel(
                                            this@PatherListViewModel,
                                            it1,
                                            typeDatas.value
                                        ))
                                    }
    ...
    fun saveItemClick(customerItemViewModel: PatherItemViewModel) {
    ...
    fun refreshItemClick() {
        try {
            clickItem?.run {
                val itemViewModel = this["itemViewModel"] as PatherItemViewModel
                addSubscribe(
                    model.getParnterSigle(itemViewModel.dataObservable.get()?.id.toString())
                        .compose(RxUtils.exceptionTransformer())
                        .compose(RxUtils.schedulersTransformer())
                        .subscribeWith(object : AppDisposableObserver<AppResponse<PartnerListBean>>() {
                            override fun onSuccess(t: AppResponse<PartnerListBean>) {
                                t?.data.let {
