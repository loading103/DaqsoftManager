 ...
 * @describe
 */
class DeptDocContainerViewModel : ToolbarViewModel<WorkBenchRepository> {
     ...
     * 部门名称
     */
    private val employeeInfo: EmployeeInfo = Gson().fromJson(
        AesCryptUtils.decrypt(
            DataStoreUtils.getString(
                DSKeyGlobal.USER_INFO)), EmployeeInfo::class.java)
    val dept = try {
        val splits = employeeInfo.employeeOrganizationStr.split("-")
        splits[splits.size-1]
    }catch (e: Exception){
        employeeInfo.employeeOrganizationStr
    }
     ...
     * 类型
     */
    var dirTypeObservable = ObservableField<String>()
     ...
     * 更多 livData
     */
    val moreLiveData = MutableLiveData<Unit>()
