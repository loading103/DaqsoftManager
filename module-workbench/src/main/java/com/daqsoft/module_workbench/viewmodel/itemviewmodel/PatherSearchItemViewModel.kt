 ...
 * 请假列表item
 */
class PatherSearchItemViewModel(
    private val customerViewModel: PatherSearchViewModel,
    ...
    var itemBinding: ItemBinding<ItemViewModel<*>> = ItemBinding.of(
        BR.viewModel,
        R.layout.recycleview_tag_item2
    )
        ...
        if(!TextUtils.isEmpty(data.partnerTypeName)){
            datas?.forEach {
                if(it.typeName==data.partnerTypeName){
                    observableList.add(TagSearchViewModel(customerViewModel, CustomerTagListBean(it.typeName!!,it.color,0)))
                }
            }
        }

        if(!TextUtils.isEmpty(data.partnerGradeName)){
            observableList.add(TagSearchViewModel(customerViewModel, CustomerTagListBean(data.partnerGradeName!!,type = 1)))
        }
    }
