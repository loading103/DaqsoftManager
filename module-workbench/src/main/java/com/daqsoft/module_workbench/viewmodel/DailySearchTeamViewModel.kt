 ...
 */
class DailySearchTeamViewModel :  ToolbarViewModel<WorkBenchRepository>, Paging2Utils<MultiItemViewModel<*>> {
        ...
        ItemBinding.of { itemBinding, position, item ->
            when (item.itemType as String) {
                ConstantGlobal.DAILY_LIST_CONTENT -> itemBinding.set(
                    BR.viewModel,
                    R.layout.recyclerview_daily_team_search
                )
                            ...
                            .subscribeWith(object : AppDisposableObserver<AppResponse<DialyListBeanItem>>() {
                                override fun onSuccess(t: AppResponse<DialyListBeanItem>) {
                                    ...
                                    t.data?.let {
                                        ...
                                        val observableList: ObservableList<MultiItemViewModel<*>> = ObservableArrayList()
                                        it.records?.forEach{
                                            var dailyListContent = DailySearchTeamContentViewModel(this@DailySearchTeamViewModel, it)
     ...
     * 保存点击的Item信息
     */
    var itemListContentViewModel: DailySearchTeamContentViewModel ? = null
    fun setItemViewModel(dailyListContentViewModel: DailySearchTeamContentViewModel) {
