 ...
 */
class DailyTeamSearchOwnItemViewModel(
    private val dailyListViewModel: DailySearchTeamOwnViewModel,
    it: DialyRecord
) : MultiItemViewModel<DailySearchTeamOwnViewModel>(dailyListViewModel){
    ...
    val onSubmitClick = BindingCommand<Unit>(BindingAction {
        dailyListViewModel.setItemViewModel(this@DailyTeamSearchOwnItemViewModel)
        goAddView()
    })
    /**
     * 赶快去编辑(修改)
